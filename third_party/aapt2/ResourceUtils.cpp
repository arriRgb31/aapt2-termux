...
bool ParseAttributeReference(StringPiece str,
     ResourceNameRef* out_ref,
                             bool*
     out_private)
 {
  StringPiece trimmed_str(util::TrimWhitespace(str));
  ...
  if (trimmed_str.size() > 1 &&
    ...
    StringPiece type;
    StringPiece entry;
    if (!android::ExtractResourceName(trimmed_str.substr(offset, trimmed_str.size() - offset), &package,
                                      &type, &entry)) {
      return false;
    }
...
std::unique_ptr<Reference> TryParseReference(StringPiece str, bool* 
out_create) {
  ResourceNameRef ref;
  bool private_ref = false;
...
util::make_unique<Reference>(ref);
...
if (ParseAttributeReference(str, &ref, &priv)) {
    if (out_create) {
        *out_create = false;
    }

    auto value =
        util::make_unique<Reference>(
    ref,
    Reference::Type::kAttribute);

    value->private_reference = priv;
